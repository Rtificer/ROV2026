 ...
 * Minimize over T⁺, T⁻:
 *
 *     || W (w - Σᵢ (T⁺ᵢ M⁺[:,i] + T⁻ᵢ M⁻[:,i])) ||² + (T⁺ + T⁻)ᵀ Q (T⁺ + T⁻)
 *
 * Subject to:
 *
 *   - 0 ≤ T⁺ᵢ ≤ 1
 ...
 *
 * Where:
 *   - T (n x 1):       Total normalized thruster command vector with values in [-1, 1].
 *   - Tᵢ (scaler):     Each Tᵢ is the effort sent to thruster i. Equal to T⁺ᵢ + T⁻ᵢ.
 *   - T⁺, T⁻ (n x 1):  Forward and reverse thruster command vectors. Constrained to [0, 1] and [-1, 0] respectively.
 *   - M⁺, M⁻  (6 x n): Thruster mapping matrices for forward/reverse. M⁺[:,i] describes the wrench contribution when thruster i is at 1 (max forward thrust), and vice versa for M⁻[:,i] (max reverse thrust).
 *   - W (6 x 6):       Diagonal wrench weighting matrix. Scales errors in certain directions, accounting for the difference in drag present for certain axes. (Difference is thrust per direction because of the angles of the thrusters is accounted for in the mixer matricies.)
 *   - Q (n x n):       Diagonal effort weighting matrix. Scales the effort penalty for each thruster. (This is used to account for differences in the output of each thruster, but likely will exlcusivly be used if a thruster stops working mid-opertation, to allow the ROV to continue operating with the remaining thrusters, but completly stopping the use of the failed thruster.)
 *   - w (6 x 1):       The desired 6-DOF wrench vector. (a vector of 3 linear forces (x, y, z) and 3 torques (roll, pitch, yaw))
 *
 * The QP is solved using OSQP. The solution is returned in T_plus and T_minus.
 *
